    ...
    public class RealtimeEventsService : IAsyncDisposable
    {
        ...
        public async Task StartAsync()
        {
            ...
            try
            {
                ...
                _logger.LogInformation("🔌 [WMSClient] SignalR: Conectando a {Hub}", _apiBase);
                RegisterHandlers(_connection);
                await _connection.StartAsync();
                _started = true;
                _logger.LogInformation("✅ [WMSClient] SignalR CONECTADO. Connection ID: {ConnectionId}", _connection.ConnectionId);
            ...
            {
                _logger.LogError(ex, "❌ [WMSClient] SignalR falló al conectar");
                _started = false;
            }
            finally
            {
        ...
        private async Task SafeInvokeAsync(string method, string arg)
        {
            try
            {
                ...
                _logger.LogInformation("📡 [WMSClient SignalR] {Method}({Arg}) OK", method, arg);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "⚠️ [WMSClient SignalR] {Method}({Arg}) falló", method, arg);
            }
        }
        ...
        private void RegisterHandlers(HubConnection connection)
        {
            ...
            {
                _logger.LogInformation("🎬 [WMSClient SignalR] InventoryEventAdded: ID={Id}, Nombre={Nombre}", ev.Id, ev.Nombre);
                InventoryEventAdded?.Invoke(ev);
            });
            connection.On<AltaEventoModel>("SavedEventAdded", ev => 
            {
                _logger.LogInformation("💾 [WMSClient SignalR] SavedEventAdded: ID={Id}, Nombre={Nombre}", ev.Id, ev.Nombre);
                SavedEventAdded?.Invoke(ev);
            });

            connection.On<PalletEventModel>("PalletEventAdded", ev =>
            {
                _logger.LogInformation("📦 [WMSClient SignalR] PalletEventAdded: ID={Id}, Ip={Ip}", ev.Id, ev.IpCamara);
                PalletEventAdded?.Invoke(ev);
            });

            connection.On<CamionEventModel>("CamionEventAdded", ev =>
            {
                _logger.LogInformation("🚚 [WMSClient SignalR] CamionEventAdded: ID={Id}, Ip={Ip}", ev.Id, ev.IpCamara);
