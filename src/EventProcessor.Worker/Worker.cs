...
public class Worker(ILogger<Worker> logger, IHostApplicationLifetime appLifetime) : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        {
            _logger.LogInformation("⏹️  Worker cancelado por solicitud");
        }
        catch (Exception ex)
        {
    ...
    {
        _logger.LogInformation("⏸️  Iniciando parada controlada del Worker...");
        ...
        {
            _logger.LogError(ex, "⚠️  Error durante la parada controlada");
        }
    }
}
